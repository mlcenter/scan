...
************************************************************************
      ...
      INTEGER NPROB,NPAR
      PARAMETER (NPROB=73,NPAR=25)
      INTEGER IFAIL,I,IMAX,NMSFLAG,OMGFLAG,MAFLAG,MOFLAG
      INTEGER UNCERTFLAG,DIFAIL,CFLAG(5),PFLAG
...
*   Exp. constraints on sparticles (LHC)
      ...
      IF(NMSFLAG.NE.0)CALL Higgsino_CMS_Trilep(PAR,PROB)
...
*******************************************************************
      ...
      INTEGER OMGFLAG,MAFLAG,MOFLAG,PFLAG,VFLAG,CFLAG(5)
...
*********************************************************************
      ...
      DOUBLE PRECISION LUX_SI,LUX_SDn,LUX_SDp,PICO2L,PICO60,XENON
     ...
     .      x,dNdx,EMIN,NBIN
      ...
      WRITE(17,900) 2,"5.4.1      # Version number"

      IF(MGL.LT.1d3 .AND. MGL.NE.0d0)
     ...
     . WRITE(17,919) 3,"# chi2(H->ZZ) > ",chi2MAX
       IF(PROB(50).NE.0d0)
     . WRITE(17,900) 3,"# excluded by sparticle searches (LHC)"
       IF(PROB(51).NE.0d0)
     . WRITE(17,900) 3,"# excluded by ggF/bb->H/A->tautau (LHC)"
       IF(PROB(52).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H->AA->4leptons/2lept.+2b (LHC)"
       IF(PROB(53).NE.0d0)
     . WRITE(17,900) 3,"# excluded by ggF->H/A->gamgam (LHC)"
      IF(PROB(55).NE.0d0)
     ...
     . WRITE(17,900) 3,"# DMK / epsK more than 2 sigma away"
      IF(PROB(63).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H(125)->AA->4gam (LHC)"
      IF(PROB(64).NE.0d0)
     . WRITE(17,900) 3,
     ...
     .   "# ",BRBKsnunumin,"   BR(B-> Ks nu nubar)-Theor.Err."
      ...
      WRITE(17,899) 
     .   "# Exp. 2 Sigma: 3.15E-1 < RD < 4.99E-1:"
     ...
     .    "# RD* = BR[B+ -> D* tau+ nu_tau]/BR[B+ -> D* l+ nu_l]"
      WRITE(17,899) 
     .   "# Exp. 2 Sigma: 2.74E-1 < RD* < 3.34E-1:"
	...
	ENDIF
     ...
     .   XSMAX,"):"
	ENDIF
        WRITE(17,901) 30,DABS(CSNSD),"sigma_n^SD"
     ...
     .   XSMAX,"):"
	ENDIF
        WRITE(17,901) 40,DABS(CSPSD),"sigma_p^SD"
...
*  From IF(OUTFLAG.NE.1)
     ...
     .    " # (STOP/SBOTTOM MASSES)"
      ...
      WRITE(17,899) "# Intervals allowed for the most SM-like state"
      WRITE(17,899) "# in the 125.1+/-3 GeV window, from LHC:"
      WRITE(17,899) "# 0.900 < CV < 1.000 "
      WRITE(17,899) "# 0.941 < CU < 1.323 "
      WRITE(17,899) "# 0.607 < CB < 0.915 "
      WRITE(17,899) "# 0.833 < CJ < 1.050 "
      WRITE(17,899) "# 0.858 < CG < 1.020 "
      WRITE(17,899) "# 0.772 < CL < 0.970 "
...
*  From IF(OUTFLAG.NE.1)
     ...
     . "# PARAMETERS OF THE EFFECTIVE LAGRANGIAN IN THE HIGGS SECTOR"
      ...
      WRITE(17,917)," DELML",DELML
      WRITE(17,917)," X(1)",(CB(1)/CL(1))**2
      WRITE(17,917)," X(2)",(CB(2)/CL(2))**2
      WRITE(17,899) "# "
*  From IF(OUTFLAG.NE.1)
      ENDIF
      ...
      WRITE(18,900) 2,"5.4.1      # Version number"
     ...
     . 'BR(t -> ~t_2 ~chi_50)'
      ...
      WRITE(19,900) 2,"5.4.1      # Version number"
      IF(PROB(30).GT.0d0)
