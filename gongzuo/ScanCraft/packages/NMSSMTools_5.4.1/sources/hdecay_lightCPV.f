      ...
      INTEGER IND,INDA,K,L,N0,VFLAG,MIXPROB
     ...
     .                 DMB1P,DMB2P,MCHIB1,MCHIB2
      ...
      DOUBLE COMPLEX CJH,CGH,CJA,CGA,F0,FF,FS,FV,CM,FGG
...
C	Coupling to photons / gluons
      ...
      WC=CM(MW/MH)
      HC=CM(dsqrt(MHC)/MH)
      ULC=CM(dsqrt(MSU2P(1))/MH)
      URC=CM(dsqrt(MSU2P(2))/MH)
      DLC=CM(dsqrt(MSD2P(1))/MH)
      DRC=CM(dsqrt(MSD2P(2))/MH)
      T1C=CM(dsqrt(MST2P(1))/MH)
      T2C=CM(dsqrt(MST2P(2))/MH)
      B1C=CM(dsqrt(MSB2P(1))/MH)
      B2C=CM(dsqrt(MSB2P(2))/MH)
      LLC=CM(dsqrt(MSE2(1))/MH)
      LRC=CM(dsqrt(MSE2(2))/MH)
      L1C=CM(dsqrt(MSL2(1))/MH)
      L2C=CM(dsqrt(MSL2(2))/MH)

      CJH=DSQRT(dsqrt(2d0)*GF)/4d0*(CU(IND)*(FF(TC)+FF(CC))
     .                                                 +CB(IND)*FF(BC))
     .  +(gRHSUSU(IND,1,1)*FS(ULC)/MSU2P(1)
     .   +gRHSUSU(IND,2,2)*FS(URC)/MSU2P(2)
     .   +gRHSDSD(IND,1,1)*FS(DLC)/MSD2P(1)
     .   +gRHSDSD(IND,2,2)*FS(DRC)/MSD2P(2))/2d0
     .  +DCMPLX(GRHSTST(IND,1,1),GIHSTST(IND,1,1))*FS(T1C)/4d0/MST2P(1)
     .  +DCMPLX(GRHSTST(IND,2,2),GIHSTST(IND,2,2))*FS(T2C)/4d0/MST2P(2)
     .  +DCMPLX(GRHSBSB(IND,1,1),GIHSBSB(IND,1,1))*FS(B1C)/4d0/MSB2P(1)
     .  +DCMPLX(GRHSBSB(IND,2,2),GIHSBSB(IND,2,2))*FS(B2C)/4d0/MSB2P(2)
     ...
     .  +2d0/3d0*(
     .    DCMPLX(GRHSTST(IND,1,1),GIHSTST(IND,1,1))*FS(T1C)/MST2P(1)
     .   +DCMPLX(GRHSTST(IND,2,2),GIHSTST(IND,2,2))*FS(T2C)/MST2P(2)
     .   +2d0*gRHSUSU(IND,1,1)*FS(ULC)/MSU2P(1)
     .   +2d0*gRHSUSU(IND,2,2)*FS(URC)/MSU2P(2))
     .  +1d0/6d0*(
     .    DCMPLX(GRHSBSB(IND,1,1),GIHSBSB(IND,1,1))*FS(B1C)/MSB2P(1)
     .   +DCMPLX(GRHSBSB(IND,2,2),GIHSBSB(IND,2,2))*FS(B2C)/MSB2P(2)
     .   +2d0*gRHSDSD(IND,1,1)*FS(DLC)/MSD2P(1)
     .   +2d0*gRHSDSD(IND,2,2)*FS(DRC)/MSD2P(2))
...
C  * A -> gamma (Rho,omega->)Pi+Pi-
     ...
     .          *AEPRhogam+OMIX(INDA,4)*AARhogam
      GamARhogam=aux**2/(2d0**8*Pi**3*MH)*KinARhogam(MH)
...
C	Light-quark and gluon decays
     ...
     .               *dsqrt(max(0d0,1d0-4d0*(MS/MH)**2))**3
       GamHjj=CDABS(CJH)**2/(8d0*PI**3)*MH**3
     .     *AS3**2*(1d0+AS3/Pi*(95d0/4d0-3d0*7d0/6d0))
...
C	H -> cc decays
      ...
      ELSE
       ...
       DCC=CDABS(CJH)**2/(8d0*PI**3)*MH**3
     .     *(AS4**2*(1d0+AS4/Pi*(95d0/4d0-4d0*7d0/6d0))
     .      -AS3**2*(1d0+AS3/Pi*(95d0/4d0-3d0*7d0/6d0)))
     ...
     .        *QCDA((RMC/MH)**2)*dsqrt(1d0-4d0*RMC**2/MH**2)+DCC)
...
C	H -> bb decays
	...
	RATCOUP=0d0
       ...
       DBB=CDABS(CJH)**2/(8d0*PI**3)*MH**3
     .     *(ASH**2*(1d0+ASH/Pi*(95d0/4d0-5d0*7d0/6d0))
     .      -AS4**2*(1d0+AS4/Pi*(95d0/4d0-4d0*7d0/6d0)))
	...
	RATCOUP=0d0
       ...
       DBB=CDABS(CJA)**2/(8d0*PI**3)*MH**3
     .     *(ASH**2*(1d0+ASH/Pi*(97d0/4d0-5d0*7d0/6d0))
     .      -AS4**2*(1d0+AS4/Pi*(97d0/4d0-4d0*7d0/6d0)))
     ...
     .         *QCDA((RMB/MH)**2)*dsqrt(1d0-4d0*RMB**2/MH**2)+DBB)
! Modifying the shape of the phase space function, so that it looks "3-body" at low mass
       IF(MH.lt.50d0)THEN
